function solve_hilbert_matrix(n)
    ...
    %----------------------------------------------------------------------
    % The explicit inversion step (N solves) costs O(N^3) time and O(N^2)
    % memory.  For N larger than a few hundred this dominates runtime.
    % Alternatives:
    %   * Replace the exact ‖A^{-1}‖_∞ with an inexpensive estimator such as
    %     Higham & Tisseur''s block 1‑norm estimator (used by MATLAB condest).
    %   * Skip forming invA entirely and instead iterate to approximate
    %     max_{‖v‖_∞=1} ‖U\L\(P*v)‖_∞ by power iteration.
    %   * Use mixed precision plus iterative refinement to reduce memory while
    %     preserving accuracy.
    %
    % For most teaching‑scale experiments (N ≤ 15–20) the present routine
    % is adequate and keeps the algorithm transparent.
