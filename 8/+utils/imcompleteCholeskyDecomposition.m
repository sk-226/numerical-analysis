function [L, D, deltaA] = imcompleteCholeskyDecomposition(A)
    % IMCOMPLETECHOLESKYDECOMPOSITION  Perform incomplete Cholesky decomposition.
    %
    %   [L, D, deltaA] = IMCOMPLETECHOLESKYDECOMPOSITION(A) decomposes the
    ...
    %     >> [L, D, deltaA] = imcompleteCholeskyDecomposition(A);
    %     >> norm(A - (L * D * L' + deltaA), 'fro')   % should be near machine epsilon
    % -------------------------------------------------------------------------

    % --------------------------- input validation ------------------------
    if ~ismatrix(A) || size(A, 1) ~= size(A, 2)
        error('IMCOMPLETECHOLESKYDECOMPOSITION:NotSquare', ...
        'Input matrix A must be square.');
    end

    % symmetry check
    if ~isequal(A, A')
        error('IMCOMPLETECHOLESKYDECOMPOSITION:NotSymmetric', ...
    ...
    if D(1, 1) <= 0
        error('IMCOMPLETECHOLESKYDECOMPOSITION:NotPositiveDefinite', ...
